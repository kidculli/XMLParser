 ...
 * 2015-10-25
 *      - Implemented to_json and write_json methods 
 *      - Added EXIT_FAILURE and EXIT_SUCCESS return values for parse_xml and to_json 
 * 2015-10-13
...
int XMLParser::to_json(const string in_file_name, const string out_file_name)
{
    // code to mute stdout source- https://bbs.archlinux.org/viewtopic.php?id=79378
    std::streambuf* cout_sbuf = std::cout.rdbuf(); // save original sbuf
    std::ofstream   fout(out_file_name,ofstream::out);
    if (parse_XML(in_file_name) == EXIT_FAILURE)
    {
        cerr << "Unable to parse and convert XML File"<< endl; 
        return EXIT_FAILURE; 
    }
    std::cout.rdbuf(fout.rdbuf()); // redirect 'cout' to a 'fout'
    write_json(fout,root,0); 
    // output closing } 
    cout << '}'<<endl; 
...
void XMLParser::write_json(ofstream& out, XMLElement* curr, unsigned level)
{
    // temp vector to hold child vector of current element
    vector<XMLElement*> child_vec = curr->get_children();
    // declare iterator 
    vector<XMLElement*>::iterator child_it;
    // if the tag has no children or attributes we will not output value as an object, just a string for content 
    if(child_vec.empty() && curr->getAttributes().empty())
    {
        cout<< (level ? '"'+curr->get_element_name()+'"'+" : "+'"'+curr->get_str_content()+"\"" : "")<<endl; 
        return ; 
    }
    // if the current element has children but no attributes 
    else if (!child_vec.empty() && curr->getAttributes().empty())
    {
        cout<< (level ? '"'+curr->get_element_name()+"\" : {" : "{" ) << endl;
        for (child_it = child_vec.begin(); child_it!= child_vec.end(); child_it++)
        {
            write_json(out, *child_it, level + 1);
            if (child_it!= child_vec.end()-1)
            {
                cout << ",";
            }
            else {
    ...
    // if the tag has objects or children we need to write it as an object 
    else if (!curr->getAttributes().empty() || !child_vec.empty()){
        cout << (level ? '"'+curr->get_element_name()+"\" : {" : "{")<<endl; 
        map<string,string> attr_map = curr->getAttributes();
        // declare iterator 
        map<string,string>::iterator it;  
        // output the attributes
        for (it = attr_map.begin(); it!= attr_map.end(); it++) {
            cout << left << setw(14)<< "\""+it->first +"\""  << " : " << "\"" + it->second + "\"";
            if (it!=attr_map.end())
            {
                cout<<',';
            }
        ...
        if(curr->get_str_content()!="{EMPTY}")
        {
            cout<< "\"text_content\" : "<<'"'+curr->get_str_content()+"\""<<endl; 
        }  
        for (child_it = child_vec.begin(); child_it!= child_vec.end(); child_it++)
        {
            write_json(out, *child_it, level + 1);
...
int XMLParser::parse_XML(const string file_name)
{
    //open file for reading
    ifstream XML_file;
    XML_file.open(file_name);
    ...
    while (getline(XML_file, curr_line)) {
        ...
        switch(curr_state)
        {
            ...
            case ELEMENT_OPENING_TAG :
                ...
                {
                    // check if root has been closed already 
                    if (unclosed_element_stack.empty())
                    {
                        cerr<< "Multiple root elements detected" <<endl; 
                        return EXIT_FAILURE; 
                    }
            ...
            case ERROR : 
            {
                cout << right << setw(4) <<"" << "ERROR\n\n" ; break ;
                cerr << "Error encountered while parsing file"<<endl;
                return EXIT_FAILURE; 
            }
